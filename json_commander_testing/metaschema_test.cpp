...
namespace {
  ...
  void
  validate(const json& schema, const json& instance) {
  ...
  void
  expect_valid(const json& schema, const json& instance) {
    REQUIRE_NOTHROW(validate(schema, instance));
  }

  void
  expect_invalid(const json& schema, const json& instance) {
    REQUIRE_THROWS(validate(schema, instance));
  }

  json
  app(json extra = json::object()) {
...
TEST_CASE("Missing required fields are rejected", "[metaschema]") {
  ...
  SECTION("missing name") {
    expect_invalid(schema, {{"doc", {"A simple application"}}});
  }
...
TEST_CASE("Invalid name formats are rejected", "[metaschema]") {
  ...
  SECTION("name starting with digit") {
    expect_invalid(schema, app({{"name", "1app"}}));
  }

  SECTION("name with spaces") {
    expect_invalid(schema, app({{"name", "my app"}}));
  }
...
TEST_CASE("Version field", "[metaschema]") {
  ...
  SECTION("two-part version") {
    expect_valid(schema, app({{"version", "1.0"}}));
  }

  SECTION("invalid version string") {
    expect_invalid(schema, app({{"version", "abc"}}));
  }
}
...
TEST_CASE("doc_string format", "[metaschema][doc_string]") {
  ...
  SECTION("single-line doc") {
    expect_valid(schema, app({{"doc", {"Short description"}}}));
  }
  ...
  SECTION("empty array is valid") {
    expect_valid(schema, app({{"doc", json::array()}}));
  }
...
TEST_CASE("Flag argument", "[metaschema][args][flag]") {
  ...
  SECTION("flag missing names") {
    expect_invalid(
      schema, app({{"args", {{{"kind", "flag"}, {"doc", {"Be verbose"}}}}}}));
  }

  SECTION("flag missing doc") {
    expect_invalid(
      schema, app({{"args", {{{"kind", "flag"}, {"names", {"verbose"}}}}}}));
  }
  ...
  SECTION("flag with empty names array") {
    expect_invalid(
      ...
      app(
        {{"args",
          {{{"kind", "flag"}, {"names", json::array()}, {"doc", {"x"}}}}}}));
  }
}
...
TEST_CASE("Positional argument", "[metaschema][args][positional]") {
  ...
  SECTION("positional missing type") {
    expect_invalid(
      ...
      app(
        {{"args",
          {{{"kind", "positional"}, {"name", "input"}, {"doc", {"Input"}}}}}}));
  }
}
...
TEST_CASE("Subcommands", "[metaschema][commands]") {
  ...
  SECTION("simple subcommand") {
    expect_valid(
      schema,
      app(
        {{"commands", {{{"name", "build"}, {"doc", {"Build the project"}}}}}}));
  }
...
TEST_CASE("Exit code info", "[metaschema][exits]") {
  ...
  SECTION("exit code out of range") {
    expect_invalid(
      schema, app({{"exits", {{{"code", 256}, {"doc", {"Too high"}}}}}}));
    expect_invalid(
      schema, app({{"exits", {{{"code", -1}, {"doc", {"Negative"}}}}}}));
  }
}
