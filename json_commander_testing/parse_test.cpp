...
TEST_CASE(
  "parse::Error is catchable as std::runtime_error", "[parse][phase1]") {
  bool caught = false;
  try {
...
TEST_CASE("ParseResult holds ParseOk", "[parse][phase1]") {
  parse::ParseResult result =
    parse::ParseOk{json({{"verbose", true}}), {"config", "set"}};
...
TEST_CASE("ParseResult holds HelpRequest", "[parse][phase1]") {
  ...
  REQUIRE(
    std::get<parse::HelpRequest>(result).command_path ==
...
TEST_CASE("ParseResult holds ManpageRequest", "[parse][phase1]") {
  ...
  REQUIRE(
    std::get<parse::ManpageRequest>(result).command_path ==
    std::vector<std::string>{"sub"});
}
...
namespace {
  ...
  arg::OptionSpec
  make_option(
    model::ArgNames names, model::TypeSpec type = model::ScalarType::String) {
    model::Option o{};
  ...
  arg::PositionalSpec
  make_positional(
    std::string name, model::TypeSpec type = model::ScalarType::String) {
    model::Positional p{};
  ...
  arg::FlagGroupSpec
  make_flag_group(
    std::string dest, std::vector<arg::FlagGroupEntrySpec> entries) {
...
TEST_CASE(
  "build_index: flag with both long and short names", "[parse][phase2]") {
...
TEST_CASE(
  "build_index: flag group entries with entry indices", "[parse][phase2]") {
...
TEST_CASE("classify_token: --verbose is LongOption", "[parse][phase3]") {
  REQUIRE(
    parse::detail::classify_token("--verbose") ==
    parse::detail::TokenKind::LongOption);
}

TEST_CASE("classify_token: --foo=bar is LongOption", "[parse][phase3]") {
  REQUIRE(
    parse::detail::classify_token("--foo=bar") ==
    parse::detail::TokenKind::LongOption);
}

TEST_CASE("classify_token: -v is ShortGroup", "[parse][phase3]") {
  REQUIRE(
    parse::detail::classify_token("-v") ==
    parse::detail::TokenKind::ShortGroup);
}

TEST_CASE("classify_token: -abc is ShortGroup", "[parse][phase3]") {
  REQUIRE(
    parse::detail::classify_token("-abc") ==
    parse::detail::TokenKind::ShortGroup);
}

TEST_CASE("classify_token: -- is DoubleDash", "[parse][phase3]") {
  REQUIRE(
    parse::detail::classify_token("--") ==
    parse::detail::TokenKind::DoubleDash);
}

TEST_CASE("classify_token: bare word is Positional", "[parse][phase3]") {
  REQUIRE(
    parse::detail::classify_token("hello") ==
    parse::detail::TokenKind::Positional);
}

TEST_CASE("classify_token: bare - is Positional", "[parse][phase3]") {
  REQUIRE(
    parse::detail::classify_token("-") == parse::detail::TokenKind::Positional);
}
...
TEST_CASE(
  "split_long_option: --foo returns (foo, nullopt)", "[parse][phase3]") {
...
TEST_CASE(
  "split_long_option: --foo=bar returns (foo, bar)", "[parse][phase3]") {
...
TEST_CASE(
  "split_long_option: --foo=bar=baz returns (foo, bar=baz)",
  "[parse][phase3]") {
...
TEST_CASE(
  "parse: flag with both names works via long form", "[parse][phase4]") {
  auto root = make_root("tool");
  root.args = {arg::ArgSpec{make_flag({"verbose", "v"})}};
  auto result = parse::parse(root, {"--verbose"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
...
TEST_CASE("parse: unknown flag throws Error", "[parse][phase4]") {
  auto root = make_root("tool");
  REQUIRE_THROWS_AS(
    parse::parse(root, {"--unknown"}, parse::no_env()), parse::Error);
}

TEST_CASE(
  "parse: repeated flag (repeated=false) sets true", "[parse][phase4]") {
  auto root = make_root("tool");
  root.args = {arg::ArgSpec{make_flag({"verbose"})}};
...
TEST_CASE("parse: repeated flag (repeated=true) counts", "[parse][phase4]") {
  ...
  auto result =
    parse::parse(root, {"--verbose", "--verbose", "-v"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
...
TEST_CASE(
  "parse: --count abc with int converter throws Error", "[parse][phase5]") {
  auto root = make_root("tool");
  root.args = {arg::ArgSpec{make_option({"count"}, model::ScalarType::Int)}};
  REQUIRE_THROWS_AS(
    parse::parse(root, {"--count", "abc"}, parse::no_env()), parse::Error);
}
...
TEST_CASE("parse: missing value at end throws Error", "[parse][phase5]") {
  ...
  REQUIRE_THROWS_AS(
    parse::parse(root, {"--output"}, parse::no_env()), parse::Error);
}
...
TEST_CASE(
  "parse: absent option without default is absent from config",
  "[parse][phase5]") {
  auto root = make_root("tool");
  root.args = {arg::ArgSpec{make_option({"output"})}};
  auto result = parse::parse(root, {}, parse::no_env());
...
TEST_CASE("parse: repeated option collects array", "[parse][phase5]") {
  ...
  root.args = {arg::ArgSpec{opt}};
  auto result =
    parse::parse(root, {"--include", "a", "-I", "b"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
...
TEST_CASE("parse: non-repeated option last wins", "[parse][phase5]") {
  ...
  root.args = {arg::ArgSpec{make_option({"output"})}};
  auto result =
    parse::parse(root, {"--output", "a", "--output", "b"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
...
TEST_CASE(
  "parse: -ov file.txt where o=option in non-final position throws",
  "[parse][phase6]") {
  auto root = make_root("tool");
  root.args = {
    arg::ArgSpec{make_option({"output", "o"})},
  ...
  REQUIRE_THROWS_AS(
    parse::parse(root, {"-ov", "file.txt"}, parse::no_env()), parse::Error);
}
...
TEST_CASE("parse: repeated flag group collects array", "[parse][phase7]") {
  ...
  auto result =
    parse::parse(root, {"--alpha", "--beta", "--alpha"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
...
TEST_CASE("parse: repeated positional collects array", "[parse][phase8]") {
  ...
  auto result =
    parse::parse(root, {"a.txt", "b.txt", "c.txt"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
...
TEST_CASE("parse: mixed options and positionals", "[parse][phase8]") {
  ...
  auto result = parse::parse(
    root, {"--verbose", "input.txt", "-o", "out.txt"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
  REQUIRE(ok.config["verbose"] == true);
...
TEST_CASE("parse: too many positionals throws Error", "[parse][phase8]") {
  ...
  root.args = {arg::ArgSpec{make_positional("file")}};
  REQUIRE_THROWS_AS(
    parse::parse(root, {"a", "b"}, parse::no_env()), parse::Error);
}
...
TEST_CASE(
  "parse: nested subcommands produce multi-element path", "[parse][phase9]") {
  auto root = make_root("tool");
...
TEST_CASE(
  "parse: --help returns HelpRequest with empty path", "[parse][phase10]") {
  auto root = make_root("tool");
...
TEST_CASE(
  "parse: -h after subcommand returns HelpRequest with path",
  "[parse][phase10]") {
  auto root = make_root("tool");
...
TEST_CASE(
  "parse: --help after subcommand returns HelpRequest with path",
  "[parse][phase10]") {
  auto root = make_root("tool");
  root.commands = {make_command("build")};
...
TEST_CASE(
  "parse: --version with no version defined throws Error", "[parse][phase10]") {
  auto root = make_root("tool");
  REQUIRE_THROWS_AS(
    parse::parse(root, {"--version"}, parse::no_env()), parse::Error);
}
...
TEST_CASE(
  "parse: --help-man returns ManpageRequest with empty path",
  "[parse][phase10]") {
  auto root = make_root("tool");
...
TEST_CASE(
  "parse: --help-man after subcommand returns ManpageRequest with path",
  "[parse][phase10]") {
  auto root = make_root("tool");
  root.commands = {make_command("build")};
...
TEST_CASE(
  "parse: --help-man short-circuits (no validation)", "[parse][phase10]") {
  auto root = make_root("tool");
  auto opt = make_option({"required-opt"});
  opt.validator = validate::required();
  root.args = {arg::ArgSpec{opt}};
...
TEST_CASE(
  "parse: --help-man in nested subcommand propagates full path",
  "[parse][phase10]") {
  auto root = make_root("tool");
  auto set_cmd = make_command("set");
  auto config_cmd = make_command("config");
  ...
  root.commands = {config_cmd};
  auto result =
    parse::parse(root, {"config", "set", "--help-man"}, parse::no_env());
  REQUIRE(std::holds_alternative<parse::ManpageRequest>(result));
  REQUIRE(
    std::get<parse::ManpageRequest>(result).command_path ==
...
TEST_CASE("parse: CLI takes precedence over env", "[parse][phase11]") {
  ...
  root.args = {arg::ArgSpec{opt}};
  auto result = parse::parse(
    root, {"--output", "cli.txt"}, make_env({{"OUTPUT", "env.txt"}}));
  auto& ok = std::get<parse::ParseOk>(result);
...
TEST_CASE("parse: env conversion error throws Error", "[parse][phase11]") {
  ...
  root.args = {arg::ArgSpec{opt}};
  REQUIRE_THROWS_AS(
    parse::parse(root, {}, make_env({{"COUNT", "abc"}})), parse::Error);
}
...
TEST_CASE(
  "integration: nested config set key value with global --verbose",
  "[parse][phase13]") {
  auto set_cmd = make_command("set");
  set_cmd.args = {
    arg::ArgSpec{make_positional("key")},
  ...
  auto result = parse::parse(
    root,
    {"--verbose", "config", "set", "user.name", "Alice"},
    parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
  REQUIRE(ok.command_path == std::vector<std::string>{"config", "set"});
  REQUIRE(ok.config["verbose"] == true);
...
TEST_CASE(
  "integration: empty argv with all-optional args", "[parse][phase13]") {
  auto root = make_root("tool");
  auto opt = make_option({"output"});
  opt.default_value = json("stdout");
  root.args = {
...
TEST_CASE("integration: all arg types combined", "[parse][phase13]") {
  ...
  auto result = parse::parse(
    root, {"-fv", "--output=out.txt", "input.txt"}, parse::no_env());
  auto& ok = std::get<parse::ParseOk>(result);
  REQUIRE(ok.config["force"] == true);
  REQUIRE(ok.config["level"] == "verbose");
  REQUIRE(ok.config["output"] == "out.txt");
...
TEST_CASE(
  "integration: idempotence (same args twice give same result)",
  "[parse][phase13]") {
  auto root = make_root("tool");
  root.args = {
    arg::ArgSpec{make_flag({"verbose", "v"})},
